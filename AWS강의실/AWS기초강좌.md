...
* 오토스케일링의 구성
    * 시작 구성(launch configurations)/시작 템플릿(launch template) : 무엇을 실행시킬 것인가에 대한 설정
        ...
        * AMI
        * 보안 그룹, key, IAM
        * 사용자 데이터 (EC2인스턴스가 올라갈 때 기본으로 런치할 코드?)
    * 모니터링 : 언제 실행시킬 것인가?
...
* 로드밸런싱
    * 스케일 아웃하여 운영할때 유저는 모든 인스턴스의 주소를 알아야 한다. 
    * 또한 관리가 어렵다
    * 로드밸런싱은 이를 해주는 것 (유저가 하나의 주소로 접근하면 알아서 인스턴스들에 나눠서 트래픽을 전송한다)
    
* Elastic Load Balancer
    * 다수의 서비스에 트래픽을 분산시켜주는 서비스
    * Health Check : 직접 트래픽을 발생시켜 인스턴스 체크
    * 여러 가용영역에 분산 가능
    * 지속적으로 IP 주소가 바뀌며 IP 고정이 불가능하다 : 항상 도메인 기반 사용
    
* 종류
    * Application Load Balancer(ALB) : 트래픽을 모니터링 하여 라우팅 가능
    * Network Load Balancer : TCP기반 빠른 트래픽 분산, Elastic IP 할당이 가능하다
    * Classic Load Balancer : 옛날에 사용하던 타입으로 레거시다
    * Gateway Load Balancer : 먼저 트래픽을 체크, 가상 어플라이언스 배포/확장 관리를 위한 서비스 (EC2에 트래픽이 도착하기 전에 인증, 로깅 ,트래픽 분석등을 위한 처리를 하는 것)

* 대상 그룹(Target Group)
    * ALB가 라우팅할 대상의 집합
    * 구성
        * 3+1가자 종류 : Instance, IP, Lambda, ALB(다른 ALB도 라우팅 가능)
        * 프로토콜 : HTTP, HTTPS, gRPC 등
        * 기타 설정 : 트래픽 분산 알고리즘, 고정 세션 등
    * ALB가 주소, 포트를 읽어서 어떤 대상 그룹으로 보낼지 결정
    * 오토스케일링 자체를 대상 그룹에 등록하여 EC2 인스턴스가 늘어나면 자동으로 대상 그룹에 추가시킬 수 있다
    * 만약 오토스케일링 그룹에서는 EC2가 정상작동중인데 EC2 내부의 웹서버가 꺼져있다면 ELB는 해당 EC2 접근시 502 error가 나타난다.(트래픽을 보내도 응답이 없음) 즉 오토스케일링 그룹과 ELB 대상 그룹의 healthy에서 차이가 있는데 이를 보완하기 위해서는 오토 스케일링 그룹을 ELB의 상태도 보도록 설정해야 한다.
